...
def compare_alike():
    def compare_sparse_matrice(A, B):
        ...
        for i in range(len(A.indptr) - 1):
            ...
            np.testing.assert_equal(A.indices[A_s:A_e][A_sorted],
                                    B.indices[B_s:B_e][B_sorted])
            np.testing.assert_equal(B.data[B_s:B_e][B_sorted],
...
def minimal_temp_network():
    # This fixture should return a ContTempNetwork instance as expected by
    # flowstab. Might need adjustment if ContTempNetwork needs arguments.
